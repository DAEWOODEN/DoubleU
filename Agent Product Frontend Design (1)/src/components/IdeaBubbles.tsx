...
export function IdeaBubbles() {
  ...
  const handleDragEnd = (idea: Idea, event: any, info: any) => {
    ...
    if (storageButtonRef.current && !idea.inStorage) {
      ...
      // More sensitive drop detection - expand the drop zone
      const dropZoneExpansion = 150;
  ...
  const getDisplaySize = (idea: Idea) => {
    ...
    // Auto-scale based on number of active ideas
    const activeIdeas = ideas.filter((i) => !i.inStorage);
    const scaleFactor = Math.max(0.4, 1 - activeIdeas.length * 0.06);
    return idea.size * scaleFactor;
  };
      ...
      <AnimatePresence>
        {activeIdeas.map((idea) => {
          ...
          return (
            ...
            >
              <motion.div
                className="relative w-full h-full rounded-full border border-border bg-background/80 backdrop-blur-sm flex items-center justify-center shadow-sm"
                style={{
                  padding: "24px",
                }}
                animate={{
              ...
              >
                <p
                  className="text-foreground text-center leading-relaxed overflow-hidden text-ellipsis"
                  style={{
                    fontSize: "14px",
                    lineHeight: "1.5",
                  }}
